  ...
  window._afTimer = setTimeout(async function() {
    ...
    try {
      var { data: unit } = await sb.from('units')
        .select('monthly_rent,rent1,rent2,tenant_name,tenant_name2')
      ...
      if(unit.tenant_name2) {
        // Two tenants - show selection
        if(badge) {
          badge.style.display = 'block';
          badge.innerHTML = '👥 ' + unit.tenant_name + ' & ' + unit.tenant_name2 + ' — اختر من يدفع';
        }
      } else {
        if(amtEl && !amtEl.value) amtEl.value = unit.monthly_rent || unit.rent1 || '';
        if(badge) {
          badge.style.display = unit.tenant_name ? 'block' : 'none';
          badge.textContent = '👤 ' + (unit.tenant_name || '');
        }
      }
